// Clean database and populate with Chapter 1 & 2 NFTs only
...
async function cleanAndPopulate() {
  ...
  try {
    ...
    console.log('📦 Adding Chapter 2 NFTs (21-40)...');
    for (let i = 21; i <= 40; i++) {
      const layers = [{layer: 0, type: 'base_artwork', description: 'Original artwork'}];

      // Add puzzle layers for special NFTs
    ...
    console.log(`\n✅ Success! Database now has ${count.total} NFTs (Chapter 1 & 2 only)`);
