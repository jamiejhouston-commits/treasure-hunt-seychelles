/**
 * Seed Treasure Hunt Chapter 2 NFTs into Database
 * Loads 20 NFTs (token IDs 21-40) with puzzle layers and art rarity from metadata files
 */
...
async function seedTreasureHuntChapter2() {
  console.log('🏴‍☠️ Starting Treasure Hunt Chapter 2 NFT seeding...\n');

  try {
    // Check if NFTs already exist
    const existingCount = await db('nfts')
      .where('chapter', 'like', '%Chapter 2%')
      .where('description', 'like', '%Treasure Hunt%')
      .count('* as count')
      .first();

    if (existingCount.count >= TOTAL_NFTS) {
      console.log(`⚠️  Found ${existingCount.count} existing Chapter 2 Treasure Hunt NFTs.`);
    ...
    // Read all 20 metadata files (nft_21.json to nft_40.json)
    for (let i = 21; i <= 40; i++) {
      ...
      const nftRecord = {
        token_id: i, // IDs 21-40
        nftoken_id: `MINTED_TESTNET_CH2_${String(i).padStart(3, '0')}`, // Testnet placeholder
        name: metadata.name,
        description: metadata.description,
        image_uri: `ipfs://PLACEHOLDER_CH2_NFT_${i}`, // Placeholder IPFS URI (will be updated during minting)
        metadata_uri: `http://localhost:3001/treasure_hunt/chapter2/metadata/nft_${i}.json`, // Metadata URL
    ...
    console.log(`\n✅ Successfully seeded ${nftsToInsert.length} Treasure Hunt Chapter 2 NFTs!`);
    console.log('\nBreakdown:');
...
module.exports = seedTreasureHuntChapter2;
