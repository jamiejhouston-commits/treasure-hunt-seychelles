#!/usr/bin/env python3
"""
FINAL FIX: Cover Bel Ombre text with SOLID OPAQUE patches
"""
...
def create_final_clean_map():
    """Create map with BEL OMBRE completely covered"""
    print("Creating final map with BEL OMBRE completely covered...")
    ...
    # COVER "BEL OMBRE" TEXT WITH SOLID OPAQUE RECTANGLE
    # Based on screenshot, Bel Ombre text is around X position 25% from left, Y 35% down
    bel_text_x = x_offset + int(new_width * 0.33)  # Adjust based on actual position
    bel_text_y = y_offset + int(new_height * 0.30)

    # Draw BIG SOLID rectangle over Bel Ombre text
    # Make it match the beige/tan color of the map background
    cover_width = 150
    cover_height = 60

    # Sample the background color from the map near that area
    bg_color = (230, 215, 190, 255)  # Beige/tan matching the map background

    draw.rectangle(
        [bel_text_x - 70, bel_text_y - 20, bel_text_x + 80, bel_text_y + 40],
        fill=bg_color  # SOLID OPAQUE COLOR
    )

    # Also cover "Anse Major" text
    anse_text_x = x_offset + int(new_width * 0.18)
    anse_text_y = y_offset + int(new_height * 0.52)

    draw.rectangle(
        [anse_text_x - 60, anse_text_y - 15, anse_text_x + 70, anse_text_y + 25],
        fill=bg_color
    )
    ...
    print(f"+ FINAL map saved: {output_path}")
    print(f"+ Backend: {backend_path}")

# Create
print("\n" + "="*70)
print("FINAL FIX: COVERING BEL OMBRE WITH SOLID PATCH")
print("="*70)
create_final_clean_map()
print("="*70)
print("BEL OMBRE TEXT NOW COVERED WITH SOLID OPAQUE RECTANGLE!")
print("="*70)