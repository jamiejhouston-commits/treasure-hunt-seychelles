...
async function generateSE2Image(card) {
  console.log(`🎨 Generating SE2-${card.index.toString().padStart(2, '0')}...`);
  
  const canvas = createCanvas(2048, 2048);
  const ctx = canvas.getContext('2d');
  
  // Background gradient (Seychelles place motif)
  const gradient = ctx.createLinearGradient(0, 0, 0, 2048);
  gradient.addColorStop(0, COLORS.abyssBlue);
  gradient.addColorStop(0.5, COLORS.seaTeal);
  ...
  for (let i = 0; i < 1000; i++) {
    ctx.fillStyle = Math.random() > 0.5 ? COLORS.mist : COLORS.lanternGold;
  ...
  // Place-specific elements based on visual description
  if (card.visual.includes('moonlit') || card.visual.includes('lantern')) {
    // Moon or lantern glow
    const glowGradient = ctx.createRadialGradient(1024, 400, 0, 1024, 400, 300);
    glowGradient.addColorStop(0, COLORS.lanternGold + '80');
    glowGradient.addColorStop(1, 'transparent');
    ctx.fillStyle = glowGradient;
    ctx.fillRect(0, 0, 2048, 2048);
  }
  
  // Pirate focal elements
  ctx.fillStyle = COLORS.lanternGold;
  ctx.strokeStyle = COLORS.mist;
  ctx.lineWidth = 8;
  
  // Draw symbolic pirate elements
  if (card.visual.includes('tricorn hat') || card.visual.includes('pirate hat')) {
    // Draw tricorn hat shape
    ctx.beginPath();
    ctx.moveTo(900, 1400);
    ctx.lineTo(1148, 1400);
    ctx.lineTo(1024, 1200);
    ctx.closePath();
    ctx.fill();
    ctx.stroke();
  }
  
  if (card.visual.includes('cutlass') || card.visual.includes('dagger')) {
    // Draw cutlass/dagger
    ctx.fillRect(1200, 1300, 200, 20);
    ctx.fillRect(1390, 1280, 30, 60);
  }
  
  if (card.visual.includes('rope')) {
    // Draw rope coil
    ctx.beginPath();
    ctx.arc(800, 1600, 80, 0, Math.PI * 2);
    ctx.lineWidth = 12;
    ctx.stroke();
  }
  
  if (card.visual.includes('compass ring') || card.cipher.includes('compass')) {
    // Draw compass ring
    ctx.beginPath();
    ctx.arc(1024, 1024, 300, 0, Math.PI * 2);
    ctx.lineWidth = 6;
    ctx.strokeStyle = COLORS.lanternGold;
    ctx.stroke();
    
    // Compass ticks
    for (let i = 0; i < 8; i++) {
      const angle = (i * Math.PI * 2) / 8;
      const x1 = 1024 + Math.cos(angle) * 280;
      const y1 = 1024 + Math.sin(angle) * 280;
      const x2 = 1024 + Math.cos(angle) * 320;
      const y2 = 1024 + Math.sin(angle) * 320;
      ctx.beginPath();
      ctx.moveTo(x1, y1);
      ctx.lineTo(x2, y2);
      ctx.stroke();
  ...
  if (card.starCount > 0) {
    ...
    for (let i = 0; i < card.starCount; i++) {
  ...
  ctx.fillText(card.name, 1024, 100);
  
  // Add place text
  ctx.font = '28px serif';
  ctx.fillText(card.place.split(',')[0], 1024, 150);
  
  // Hidden letter (very faint)
  ctx.globalAlpha = 0.15;
  ctx.font = 'bold 200px serif';
  ctx.fillText(card.hiddenLetter, 1024, 1024);
  ctx.globalAlpha = 1.0;
  
  // Save image
  const filename = `se2-${card.index.toString().padStart(2, '0')}.png`;
...
async function main() {
  ...
  console.log('🎨 Phase 1: Generating 20 SE2 Images...');
  for (const card of SE2_CARDS) {
    await generateSE2Image(card);
  ...
  for (const card of SE2_CARDS) {
    ...
    manifest.push({
      index: card.index,
      filename: `se2-${card.index.toString().padStart(2, '0')}.png`,
      image_cid: imageCid,
...
`;
  ...
  console.log('========================');
  console.log(`✅ Minted 20 Special-Edition NFTs for Chapter II — The Landing at Baie Lazare`);
  console.log(`📍 Wallet: ${xrpl.Wallet.fromSeed(process.env.XRPL_TESTNET_SEED).address}`);
  console.log(`🔢 Star Offset: ${SE2_CARDS.filter(c => c.starCount > 0).map(c => c.starCount).join('')}`);
  console.log(`📊 First 3 NFTokenIDs:`);
  mintedRecords.slice(0, 3).forEach(r => {
    console.log(`   ${r.name.split(' ')[0]}: ${r.nftoken_id}`);
  });
  console.log(`📊 Last 3 NFTokenIDs:`);
  mintedRecords.slice(-3).forEach(r => {
    console.log(`   ${r.name.split(' ')[0]}: ${r.nftoken_id}`);
  });
  console.log(`\n🌐 All URIs saved to: ${mintedPath}`);
